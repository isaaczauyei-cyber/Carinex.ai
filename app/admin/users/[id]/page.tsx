...
export default async function AdminUserDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: authUser } = userInfo?.id
    ? await adminClient.auth.admin.getUserById(userInfo.id)
    : { data: null };
  const email = authUser?.user?.email || "—";

  const pendingCompletions = (completions || []).filter((c) => c.status === "verification_pending");
  const visitCount = (activity || []).length;
  const lastActive = activity && activity.length > 0 ? activity[0].activity_date : null;

  const specializations = (nurseSpecs || [])
    .map((r) => r.specializations as unknown as { name: string; slug: string })
    .filter(Boolean);
    ...
    <main>
      ...
      <section className="mx-auto max-w-3xl px-6 py-12">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-carinex-navy/10 p-4">
            <p className="text-xs font-semibold uppercase text-carinex-navy/50">Last active</p>
            <p className="mt-1 text-carinex-navy">
              {lastActive ? new Date(lastActive).toLocaleDateString() : "Never"}
            </p>
          </div>
          <div className="rounded-xl border border-carinex-navy/10 p-4">
            <p className="text-xs font-semibold uppercase text-carinex-navy/50">Days active</p>
            <p className="mt-1 text-carinex-navy">{visitCount}</p>
          </div>
        </div>

        {specializations.length > 0 && (
          <div className="mt-8">
            <h2 className="text-lg font-bold text-carinex-navy">Specializations enrolled</h2>
            <div className="mt-3 flex flex-wrap gap-2">
              {specializations.map((s) => (
                <span key={s.slug} className="rounded-full bg-carinex-emerald/10 px-3 py-1 text-xs font-semibold text-carinex-emerald">
                  {s.name}
                </span>
              ))}
            </div>
